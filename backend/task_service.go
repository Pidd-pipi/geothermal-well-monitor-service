...
func (s *TaskService) Assign(ctx context.Context, id, operator string, expected int) (Task, error) {
	...
	defer s.store.EndAssign(ctx, id)
...
func (s *TaskService) Complete(ctx context.Context, id, operator string, expected int) (Task, error) {
	select {
	case <-ctx.Done():
		return Task{}, ctx.Err()
	default:
	...
	if err := s.store.EndAssign(ctx, id); err != nil {
		return Task{}, err
	}
